...
public class Logic
{
    ...
    private IEnumerable<Point> RunI(Item[][] rows, Item[][] columns)
    {
        ...
        Rows = rows.Select((s, si) => new Line(si, true, Width, s))
        .ToFrozenDictionary(k => k.LineIndex);
        Cols = columns.Select((s, si) => new Line(si, false, Height, s))
        .ToFrozenDictionary(k => k.LineIndex);
        ...
        foreach (Line Line in ForEachLine(lines))
        {
            ...
            foreach (var (Gap, Ls, Skip) in Line.GetGaps(true))
            {
                ...
                else
                {
                    ...
                    else if (ItemE.HasValue && (ItemFillsGap(Gap, EqualityE, ItemE)
                        || (ItemE.Value.Value == Gap.Size && Index == IndexE)))
                    {
                        yield return AddPoints(Line,
                                               Gap.Start,
                                               GriddlerPath.Action.GapFull,
        ...
        foreach (Line Line in ForEachLine(lines))
        {
            ...
            foreach (var (Block, Gap, Ls, Skip) in Line.GetBlocks(true))
            {
                ...
                bool CompleteItem(out bool S, out bool E, out int? ItmIdx)
                {
                    ...
                    (S, E) = Range.ShouldAddDots(Block, EqualityIndex, EqualityIndexE);
        ...
        foreach (Line Line in ForEachLine(lines))
        {
            ...
            foreach (var (Block, Gap, Ls, Skip) in Line.GetBlocks(true))
            {
                    ...
                    && Gap.Start + m - 1 > Block.End)
                {
                    ...
                    if (PointsChange != Line.Points.Count)
                        break; //might be split block - breaks isolations
                }
                ...
                if (HalfGapOverlapBackwards(out var r))
                {
                    ...
                    {
                        int PointsChange = Line.Points.Count;
                        int DotsChange = Line.Dots.Count;
                        yield return FullPart(Line,
                                              Gap.Start,
                                              r.Start,
                                              r.End,
